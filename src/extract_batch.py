...
def main(args):
    ...
    # Build set of reads to keep if given ...
    keep_reads = set()
    if len(args.reads_to_keep) > 0:
        with open(args.reads_to_keep, "r") as input_fd:
            all_lines = [keep_reads.add(x.strip()) for x in input_fd.readlines()]
