...
def get_files_for_analyzing_document_numbers(wildcards):
    ...
    input_filelist = [f"results_exp1/{output_type}/{read_type}/dataset_{num}/dataset_{num}_{read_type}_reads.fa.doc_numbers" for num in range(0, num_datasets+1)]
...
rule produce_list_of_genomes:
    input:
        get_group_lists
    output:
        temp("step_1_exp1/dataset_{num}/dataset_{num}_list.txt")
    run:
...
rule generate_full_file_list:
    input:
        expand("step_1_exp1/dataset_{num}/dataset_{num}_list.txt", num=range(1,num_datasets+1))
    output:
        "file_list_exp1/genome_file_list.txt"
    run:
        with open(output[0], "w") as fd:
...
rule generate_one_seq_per_group:
    input:
        "step_1_exp1/dataset_{num}/dataset_{num}_list.txt"
    output:
        temp("step_2_exp1/dataset_{num}/dataset_{num}.fa")
    shell:
        "while read line; do cat $line >> {output}; done<{input} "

rule generate_rev_comp_for_group:
    input:
        "step_2_exp1/dataset_{num}/dataset_{num}.fa"
    output:
        temp("step_3_exp1/dataset_{num}/dataset_{num}_rev_comp.fa")
    shell:
...
rule generate_combined_seq_for_group:
    input:
        "step_2_exp1/dataset_{num}/dataset_{num}.fa",
        "step_3_exp1/dataset_{num}/dataset_{num}_rev_comp.fa"
    output:
...
rule build_spumoni_index:
    input:
        "file_list_exp1/genome_file_list.txt"
    output:
        "index_exp1/spumoni_full_ref.fa.thrbv.ms",
        "index_exp1/spumoni_full_ref.fa.thrbv.spumoni"
    run:
...
rule generate_raw_positive_long_reads:
    ...
    run:
        ...
        shell("ls 'step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_'*.fastq | xargs rm")
        shell("ls 'step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_'*.ref | xargs rm")
        shell("ls 'step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_'*.maf | xargs rm")

rule convert_long_reads_to_fasta_and_subset:
    input:
        "step_4_exp1/ont/dataset_{num}/dataset_{num}_ont_reads.fastq"
    output:
        "reads_exp1/ont/dataset_{num}/dataset_{num}_ont_reads.fa"
    run:
        num_lines = num_reads_per_dataset * 4
        shell("head -n{num_lines} {input} > step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_subset.fq")
        shell("seqtk seq -a step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_subset.fq > {output}")
        shell("if [ $(grep -c '>' {output}) != {num_reads_per_dataset} ]; then echo 'number of reads assertion failed.'; exit 1; fi")
        shell("rm step_4_exp1/ont/dataset_{wildcards.num}/dataset_{wildcards.num}_ont_reads_subset.fq")

rule convert_short_reads_to_fasta_and_subset:
    input:
        "step_4_exp1/illumina/dataset_{num}/dataset_{num}_illumina_reads.fq"
    output:
        "reads_exp1/illumina/dataset_{num}/dataset_{num}_illumina_reads.fa"
    run:
        num_lines = num_reads_per_dataset * 4
        shell("head -n{num_lines} {input} > step_4_exp1/illumina/dataset_{wildcards.num}/dataset_{wildcards.num}_illumina_reads_subset.fq")
        shell("seqtk seq -a step_4_exp1/illumina/dataset_{wildcards.num}/dataset_{wildcards.num}_illumina_reads_subset.fq > {output}")
        shell("if [ $(grep -c '>' {output}) != {num_reads_per_dataset} ]; then echo 'number of reads assertion failed.'; exit 1; fi")
        shell("rm step_4_exp1/illumina/dataset_{wildcards.num}/dataset_{wildcards.num}_illumina_reads_subset.fq")
...
rule generate_random_short_reads:
    output:
        "reads_exp1/illumina/dataset_0/dataset_0_illumina_reads.fa"
    shell:
        "python {repo_dir}/src/random_reads.py -n {num_reads_per_dataset} -l 150 > {output}"

rule generate_random_long_reads:
    output:
        "reads_exp1/ont/dataset_0/dataset_0_ont_reads.fa"
    shell:
...
rule copy_reads_to_result_folder:
    input:
        "reads_exp1/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa"
    output:
        "results_exp1/ms/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa",
        "results_exp1/pml/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa"
    run:
...
rule run_spumoni_ms_on_reads:
    input:
        ...
        "index_exp1/spumoni_full_ref.fa.thrbv.ms"
    output:
        "results_exp1/ms/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa.lengths",
        "results_exp1/ms/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa.doc_numbers"
    shell:
        "spumoni run -r index_exp1/spumoni_full_ref.fa -p {input[0]} -M -f -d"
...
rule run_spumoni_pml_on_reads:
    input:
        ...
        "index_exp1/spumoni_full_ref.fa.thrbv.spumoni"
    output:
        "results_exp1/pml/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa.pseudo_lengths",
        "results_exp1/pml/{type}/dataset_{num}/dataset_{num}_{type}_reads.fa.doc_numbers"
    shell:
        "spumoni run -r index_exp1/spumoni_full_ref.fa -p {input[0]} -P -f -d"
...
rule analyze_document_numbers:
    ...
    output:
        "analysis_exp1/{type}_{output}_doc_analysis.csv"
    run:
